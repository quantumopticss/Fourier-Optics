...
def _propagation_impulse(E_input,lbd,d,dx,mask = 1.0):
    ...
    if dimension == 2:
        ...
        E_propagate = 1j*np.exp(-1j*2*np.pi*d/lbd) * (M_i @ (E_input*mask) @ M_j)*(dx**2)
...
"""
def _propagation_impulse(E_input,lbd,d,dx,mask = 1):
    ...
    if dimension == 2:
        ...
        nxlist = np.arange(-N//2,N//2)
        nylist = np.arange(-M//2,M//2)
        
        xx,yy = np.meshgrid(nxlist,nylist)
        
        h = np.exp(-1j*pi*(xx**2+yy**2)*dx**2/(lbd*d))
        
        f_h = np.fft.fftshift(np.fft.fft2(h),axes = (0,1)) 
        f_E = np.fft.fftshift(np.fft.fft2(E_input),axes = (0,1))
        
        E_propagate = np.fft.ifft2(f_E*f_h)*(1j/(lbd*d))
                
    elif dimension == 1:
        N = np.size(E_input)
        nxlist = np.arange(-N//2,N//2)
        
        h = np.exp(-1j*pi*(nxlist*dx)**2/(lbd*d))
        f_h = np.fft.fftshift(np.fft.fft(h)) 
        f_E = np.fft.fftshift(np.fft.fft(E_input))
        
        E_propagate = np.ifftshift(np.sqrt(1j/(lbd*d))*np.fft.ifft(f_h*f_E),axes=(0,1))
        
    return E_propagate*np.exp(-1j*2*np.pi*d/lbd)
