...
class photonic_crystal_1d:
    def __init__(self,eta_fun,gamma,N = 128,mode = "TM",**kwargs):
        ...
        self.kb_fun = lambda x: (x*(x<1.)*(x>=0.) + 1.*(x>=1.)*(x<2.) + (3.-x)*(x>=2.)*(x<=3.))*self.g
        self.kx_fun = lambda x: (0.*(x<1.)*(x>=0.) + (x-1.)*(x>=1.)*(x<2.) + (3.-x)*(x>=2.)*(x<=3.))*self.g
    
    def photonic_crystal(self,omega_num = 3,mode_num = 0):
        sweep_list = np.linspace(1e-3,0.99,49,endpoint = False)
        
        kb_list = self.kb_fun(sweep_list)
        kx_list = self.kx_fun(sweep_list)
