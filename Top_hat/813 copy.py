...
param = [torch.tensor(1.), torch.tensor(round(150e-6/focal_spx)), torch.tensor(round(50e-6/focal_spy)), torch.tensor(-40+NT[0]/2.), torch.tensor(-40+NT[1]/2.), torch.tensor(9)]  # [d2, sigma, l, roi, roj, C1]
...
init_phi, ipp = phase_guess(N, D, 0.9, 1.3/1000, torch.tensor(mu), 0, save_param=True)# init_phi, ipp = slm.phase_guess(N, 0, 0.5, curv/1000, 0, 0, save_param=True)  # 猜测相位
...
class FourierOp:
    ...
    def perform(self, node, itorchuts, output_storage):
        x = itorchuts[0] + 1j * itorchuts[1]
        s = torch.fft.fftshift(torch.fft.fft2(torch.fft.ifftshift(x)))
        output_storage[0] = s.real
        output_storage[1] = s.imag
        z_r = output_storage[0]
    ...
    def grad(self, itorchuts, output_gradients):
        z_r = output_gradients[0]
        z_i = output_gradients[1]
        y = InverseFourierOp()(z_r, z_i)
        return y
    ...
    def __call__(self, xr, xi):
        # 创建节点
        itorchuts = self.make_node(xr, xi)
        output_storage = [torch.empty_like(xr), torch.empty_like(xi)]
        # 执行傅里叶变换
        self.perform(None, itorchuts, output_storage)
        return output_storage[0], output_storage[1]  # 返回实部和虚部
...
plt.show()
#%%
# E_out_r=E_out_r.detach().numpy()
# E_out_i=E_out_i.detach().numpy()
lambda_ = 420e-9
z = 1  # 距离焦平面外的传播距离，单位为米
n = E_in_r_pad.shape[0]
dx = spix/1000 # 像素大小 (单位：米)
# kx = torch.fft.fftfreq(n, dx)  # 空间频率 (单位：1/米)
# ky = torch.fft.fftfreq(n, dx)
# KX, KY = torch.meshgrid(kx, ky)  # 空间频率网格
# # K = torch.sqrt((2 * torch.pi / lambda_)**2 * (KX**2 + KY**2) + (2 * torch.pi / lambda_)**2)
# K = torch.sqrt((2 * torch.pi / lambda_)**2 * (KX**2 + KY**2))


kx = torch.fft.fftfreq(n, dx)
ky = torch.fft.fftfreq(n, dx)
KX, KY = torch.meshgrid(kx, ky)
KX = torch.fft.fftshift(KX)  # 频率网格平移
KY = torch.fft.fftshift(KY)
K = torch.sqrt((2 * torch.pi / lambda_)**2 * (KX**2 + KY**2))

prop_factor = torch.exp(1j * z * K)
E_out_r_z = E_out_r * prop_factor.real - E_out_i * prop_factor.imag
E_out_i_z = E_out_r * prop_factor.imag + E_out_i * prop_factor.real

# 可视化传播后的光场的强度（焦平面后电场的幅度）
E_out_2_z = E_out_r_z ** 2 + E_out_i_z ** 2
plt.imshow(E_out_2_z)
plt.colorbar()
plt.show()
# %%
i, j = 171, 171  # 选择图像中心点
...
plt.show()
# %%
def fresnel_lens_phase_generate(shift_distance, SLMRes=(1024,1272), x0=N[0]/2, y0=N[1]/2, pixelpitch=12.5,wavelength=0.813,focallength=400000,magnification=1):
        '''
        ...
        return torch.remainder(fresnel_lens_phase,2*torch.pi).cpu()
# %%
...
# 定义不同的传播距离 z 取值
z_values = torch.linspace(-100, 100, 20)  # 传播距离范围，从 0 到 2cm
