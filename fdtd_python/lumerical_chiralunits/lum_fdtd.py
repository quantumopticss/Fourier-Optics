...
class LumSimulation(BaseSimulation):
    ...
    def add_source(self,center,size,source_type,**kwargs):
        """定义光源
        parameter:
            center:  (x,y,z),中心坐标
            size: (x_span,y_span,z_span),沿x,y,z轴尺寸大小
        ...
        **kwargs：
        ...
        if source_type == 'GaussSource':
            if 'w0' in kwargs:
                ...
                if 'jones' in kwargs:
                    ...
                    if np.abs(Jones[0]) != 0:
                        ...
                        self.fdtd.set("name", (kwargs.get('name', "gausssource") + '_Ex'))
                        self.fdtd.set("amplitude", amp*np.abs(Jones[0]))
                        ...
                        self.fdtd.set("Phase", np.angle(Jones[0]))
                        self.fdtd.set("waist radius w0", w0)
                    ## Ey
                    if np.abs(Jones[1]) != 0:
                        self.fdtd.addgaussian()
                        self.fdtd.set("name", (kwargs.get('name', "gausssource") + '_Ey'))
                        self.fdtd.set("amplitude", amp*np.abs(Jones[1]))
                        self.fdtd.set("injection axis", axis)
                        self.fdtd.set("direction", direction)
                        self.fdtd.set("x", center[0])
                        self.fdtd.set("x span", size[0])
                        ...
                        self.fdtd.set("Phase", np.angle(Jones[1]))
                        self.fdtd.set("waist radius w0", w0)
                else:
                    print('Please determine Jones vector')
            else:
                print('please input w0 (waist radius)')

        elif source_type == 'PlaneSource':
            if 'jones' in kwargs:
                Jones = kwargs.get('jones',[1,0])
                # Ex
                if np.abs(Jones[0]) != 0:
                    self.fdtd.addplane()
                    self.fdtd.set("name", (kwargs.get('name', "planesource") + '_Ex') )
                    self.fdtd.set("injection axis", axis)
                    self.fdtd.set("direction", direction)
                    self.fdtd.set("amplitude", amp*np.abs(Jones[0]))
                    self.fdtd.set("x", center[0])
                ...
                if np.abs(Jones[1]) != 0:
                    self.fdtd.addplane()
                    self.fdtd.set("name", (kwargs.get('name', "planesource") +'_Ey') )
                    self.fdtd.set("injection axis", axis)
                    self.fdtd.set("direction", direction)
                    self.fdtd.set("amplitude", amp*np.abs(Jones[1]))
                    self.fdtd.set("x", center[0])
            ...
            print("Please choose PlaneSource , GaussSource or PointSource! ")
    ...
    def add_sim_area(self, center, size, **kwargs):
        ...
        **kwargs：
        ...
        self.fdtd.set('mesh accuracy', kwargs.get('mesh_accuracy', 3))
...
if __name__ == '__main__':
    unit = 1e-6
    alist = np.linspace(30,150,3)*unit/1e3
    blist = np.linspace(30,150,3)*unit/1e3
    period = 2*unit
    h = 1*unit
    ...
    for theta in anglelist:
        for a in alist:
            for b in blist:
                ...
                wl_min = 0.538 * unit
                nfreq = 1
                save_name = 'meta_units'        # 仿真文件名，不用加后缀.fsp
