...
if __name__ == '__main__':
    ...
    unit = 1e-6
    alist = np.linspace(120,150,1)*unit/1e3
    blist = np.linspace(30,150,1)*unit/1e3
    period = 2*unit
    h = 1*unit
    ...
    for theta in anglelist:
        for a in alist:
            for b in blist:
                ...
                # 创建超表面单元
                unity_sim.add_block(size=(120/1e9, 30/1e9, h), center=(0, 0, h/2), material_name='TiO2', name='meta1', color='Red')
                unity_sim.add_block(size=(30/1e9, 45/1e9, h), center=(0, 22.5/1e9, h/2), material_name='TiO2', name='meta2', color='Red')
                unity_sim.add_block(size=(30/1e9, 45/1e9, h), center=(0, -22.5/1e9, h/2), material_name='TiO2', name='meta3', color='Red')

                # 添加源和监视器
                unity_sim.add_source(size=(period, period, 0), center=(0, 0, source_z),amplitude = 1, jones = [np.cos(theta),np.sin(theta)], source_type = "PLaneSource",name = '1')
                ...
                nx = result_i['Ex']
                ny = result_i['Ey']
                Ne = np.sum(np.abs(nx)**2+np.abs(ny)**2,axis = None)/np.size(nx)

                Ex_angle = (ex*np.cos(theta) + ey*np.sin(theta))/Ne
                Ey_angle = (ey*np.cos(theta) - ex*np.sin(theta))/Ne
                ...
                np.savetxt(("results/" + "EXresult" + f"_theta = {theta:.2f},a = {(a*1e9):.2f},b = {(b*1e9):.2f}.txt"), Ex_angle)
                np.savetxt(("results/" + "EYresult" + f"_theta = {theta:.2f},a = {(a*1e9):.2f},b = {(b*1e9):.2f}.txt"), Ey_angle)
